$(document).ready(function() {
  ...
  $('#btn_agregar').on('click', function (evento) {
    ...
    $.ajax({
	  ...
	  var val_valor = val_parciales * val_tipo_cambio ;
	  val_valor = Math.round(val_valor * 100 ) / 100 ;
	  var val_igv = val_valor * 0.18 ; 
	  val_igv = Math.round(val_igv * 100) / 100 ;
	  
	  var val_importe = val_valor + val_igv ;
	  val_importe = Math.round(val_importe * 100) / 100;
	  ...
	  var total_costo = (parseFloat($('#id_otros_costos_MN').val()) + total_valor );
	  var total_pagar = total_importe + parseFloat($('#id_percepcion_MN').val());
	  
	  var ME_total_importe = total_importe / val_tipo_cambio ;
	  var ME_total_valor = (total_valor / val_tipo_cambio);
	  var ME_total_igv = total_igv / val_tipo_cambio ;
	  ...
	  // td costo
	  var val_costo = (val_importe * total_costo) / total_importe;
  ...
  $('.table tbody').on('click', 'a#eliminar',function () {
    ...
    valor =  row.find("td").eq(7).html();
    igv = row.find("td").eq(8).html();
    total_costo = row.find("td").eq(9).html();

    var val_tipo_cambio = parseFloat($('#id_tipo_cambio').val());    
    
    var total_importe = parseFloat($('#id_total_importe').val()) - parseFloat(importe);
    var total_parcial = total_importe / val_tipo_cambio;
