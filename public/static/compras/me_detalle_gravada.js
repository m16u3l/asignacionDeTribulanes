$(document).ready(function() {
  ...
  $('#btn_agregar').on('click', function (evento) {
    ...
    $.ajax({
	  ...
	  var val_exonera = parseFloat($('#id_no_gravada').val()) * val_tipo_cambio;
	  val_exonera = Math.round(val_exonera * 100) / 100;
	  var val_valor = (val_importe - val_exonera) / 1.18;
	  ...
	  var total_exonera = parseFloat($('#id_total_no_gravada').val()) + val_exonera;
	  ...
	  var total_costo = (parseFloat($('#id_otros_costos_MN').val()) + total_exonera + total_valor);
	  ...
	  var ME_total_valor = (ME_total_importe - ME_total_no_gravada) / 1.18;
	  var ME_total_igv = ME_total_valor * 0.18;
	  ...
	  ME_total_no_gravada = Math.round(ME_total_no_gravada * 100 ) / 100 ;
  	  ME_total_valor = Math.round(ME_total_valor *100) / 100;
	  ME_total_igv = Math.round(ME_total_igv *100) / 100;
	  ...
	  $('#id_ME_total_importe').val(ME_total_importe);
  	  $('#id_ME_total_igv').val(ME_total_igv);
  	  $('#id_ME_total_valor').val(ME_total_valor);
  ...
  $('.table tbody').on('click', 'a#eliminar',function () {
    ...
    valor =  row.find("td").eq(8).html();
    igv = row.find("td").eq(9).html();
    total_costo = row.find("td").eq(10).html();    
    ...
    var total_pagar = total_importe + parseFloat($('#id_percepcion_MN').val());    
    var ME_total_no_gravada = total_exonera / val_tipo_cambio;
    var ME_total_valor = (total_parcial - ME_total_no_gravada) / 1.18;
    var ME_total_igv = ME_total_valor * 0.18;
  ...
  $('#id_no_gravada').on('keypress', function(e) {
    var keyCode = e.keyCode || e.which;
    if (keyCode === 13) { 
      e.preventDefault();
      $('#btn_agregar').click();
  ...
  $('#formulario_movimiento select#id_documento').on('change', function(e) {
    $('#formulario_movimiento input#id_numero').focus();
  });

  $('#formulario_movimiento select#id_comprobante').on('change', function(e) {
    $('#formulario_movimiento input#id_serie').focus();
  });  
  
  $('#formulario_movimiento select#id_medio_pago').on('change', function(e) {
    $('#formulario_movimiento input#id_percepcion_ME').focus();
  });

  $('#formulario_movimiento select#id_proveedor').on('change', function(e) {
    $('#formulario_movimiento input#id_pago_cuenta').focus();
  });
